...
export default function DataCard({ data }) {
    ...
    export default function DataCard({ data }) {
        ...
        return (
            <div className="bg-primary-bg-cards rounded-2xl p-4 border border-primary-border shadow-sm max-w-sm w-full antialiased text-primary-text font-label select-none">
                <div className="mb-4">
                ...
                <div className="space-y-3.5">
                    {showData.map((item, index) => (
                        <div key={index} className="space-y-1.5">
                            ...
                            <span className="font-semibold text-primary-text whitespace-normal break-words leading-tight pt-0.5">
                                {item.categorie}
                            </span>

                                <span className="font-bold text-primary-text bg-primary-bg px-2 py-0.5 rounded text-[11px] shrink-0 tracking-wide border border-primary-border/60 shadow-sm font-mono">
                                {item.aantal}x ({item.percentage}%)
                            </span>
                            </div>

                            <div className="w-full h-2 bg-primary-bg rounded-full overflow-hidden border border-primary-border/20">
                                <div
                                    className={`h-full rounded-full ${getColorClass(item.type)} transition-all duration-700 ease-out`}
