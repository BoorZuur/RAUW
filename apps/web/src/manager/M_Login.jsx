...
export default function ManagerLogin() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Verzoek naar het specifieke management-login endpoint
            const response = await axios.post('http://localhost:8001/api/auth/manager/login', {
                email: email,
                password: password,
            }, {
                headers: {
            ...
            if (response.data.token) {
                localStorage.setItem('manager_auth_token', response.data.token);
            }
            navigate('/manager/dashboard');
        } catch (err) {
            if (err.response && err.response.status === 401) {
                setError('Beheergegevens zijn onjuist.');
            ...
            >
                ...
                <div className="max-w-md text-center z-10 text-(--primary-text-d)">
                    <h1 className="text-4xl lg:text-5xl tracking-tight leading-[1.15] mb-6 whitespace-pre-line text-balance font-(--font-family-headline)">
                        Regie &<br/>Beheer van<br/>veiligheid.
                    </h1>
