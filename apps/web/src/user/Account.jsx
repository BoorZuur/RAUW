...
import { Settings, MapPin, CheckCircle, MessageSquare, BookOpen, User } from 'lucide-react';
...
export default function Dashboard() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            } catch (err) { console.error("Kon de buurtgegevens niet ophalen"); }
        };
        fetchData();
    }, []);
    ...
    return (
        <div className="min-h-screen bg-primary-bg transition-colors duration-300">
            <div className="fixed top-0 w-full z-50">
                ...
                <U_Nav currentTheme={isDark ? 'dark' : 'light'} toggleTheme={toggleTheme} />
            </div>
            ...
            <main className="pt-32 p-8 max-w-4xl mx-auto space-y-8">
                ...
                <section className="flex items-center justify-between p-4 bg-primary-bg-cards border border-primary-border rounded-2xl">
                    ...
                    <Link to="/instellingen" className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-primary-text hover:bg-secondary-bg rounded-lg transition-all">
                ...
                <section>
                    ...
                    <div className="space-y-4">
                        {activeTab === 'verhalen' ? (
                            <div className="p-10 w-full border border-primary-border rounded-xl bg-primary-bg-cards flex flex-col items-center text-center">
                                ...
                                <p className="text-primary-text/70 max-w-sm">
                                    Hier vind je binnenkort updates over buurtinitiatieven en verhalen die je met een hartje hebt gemarkeerd.
                                </p>
                            </div>
                        ) : filteredReports.length > 0 ? (
                            filteredReports.map(report => (
                                <div key={report.id} className="p-6 bg-primary-bg-cards rounded-xl border border-primary-border flex items-start gap-5 hover:border-primary-text/30 transition-all">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        {report.status === 'afgehandeld' && (
                                            <div className="mt-3 flex items-center gap-2 text-green-700 font-bold text-sm">
                            ...
                            <div className="p-10 text-center border-2 border-dashed border-primary-border rounded-xl bg-primary-bg-cards">
                                <MessageSquare className="w-8 h-8 mx-auto text-primary-text/30 mb-3" />
                                <p className="text-primary-text/70">Geen meldingen in deze categorie. Alles rustig in de buurt?</p>
                            </div>
                        )}
                    </div>
                </section>
