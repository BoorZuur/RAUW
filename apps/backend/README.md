...
**List filters:** `followed=1` (users only) returns a deduped list of canonical stories the user follows: when the user owns a duplicate child and still participates on the canonical, the child row is returned (full owner payload); otherwise the canonical row is returned (participant redaction via `IssueResource`). Own canonical reports are excluded (they appear under `mine=1` only). `participating=1` (users only) lists owned duplicate children where the user still participates on the canonical. `include_duplicates=1` (officers/managers only) includes duplicate child rows. Default browse hides other users' duplicate children for citizens and hides all children for officers/managers unless `include_duplicates=1`. `mine`, `participating`, and `followed` are pairwise mutually exclusive.
